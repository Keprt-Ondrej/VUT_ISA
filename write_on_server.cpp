...
int write_on_server_main(std::string &path,int timeout,int size,bool b_multicast,std::string&mode, std::string &ip,std::string &port){
    ...
    FILE *file = open_file(path,mode,'r');
    if (file == NULL){
    ...
    packet.clear_buffer();    
    received_data = klient.receive_msg(packet.buffer_size,packet.buffer);
    if (received_data == -1){
    ...
    if (opcode == TFTP_ACK && block_ack == 0){
        while (data_read >= block_size ){
            ...
            received_data = klient.receive_msg(packet_ack.buffer_size,packet_ack.buffer);
